...
async def exchange_rates(message: Message):
    url = "https://v6.exchangerate-api.com/v6/8da1e13bfad34940c93cfc32/latest/USD"
    try:
...
async def show_users(message: Message):
    ...
    # Если записи найдены, формируем сообщение для отправки
    if users:
        response = "Список пользователей:\n\n"
        for user in users:
            user_id, telegram_id, name, category1, category2, category3, expenses1, expenses2, expenses3 = user
            response += (f"ID: {user_id}\n"
                         f"Telegram ID: {telegram_id}\n"
                         f"Имя: {name}\n"
                         f"Категория 1: {category1 or 'Не указано'}, Расходы: {expenses1 or 0}\n"
                         f"Категория 2: {category2 or 'Не указано'}, Расходы: {expenses2 or 0}\n"
                         f"Категория 3: {category3 or 'Не указано'}, Расходы: {expenses3 or 0}\n\n")
    else:
